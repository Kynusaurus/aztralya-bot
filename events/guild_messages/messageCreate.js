...
module.exports = {
    ...
    async execute(client, message) {
        ...
        if (cmd.ownerOnly) {
            if (message.author.id != ownerId) return message.reply('La seule personne pouvant tapper cette commande est l\'owner du bot');
        }

        if (!message.member.permissions.has([cmd.permissions])) return message.reply(`Vous avez pas les permissions(\`${cmd.permissions.join(', ')}\`) pour utiliser cette commandes !`);
