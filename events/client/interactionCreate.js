...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (interaction.isCommand() || interaction.isContextMenu()) {
            ...
            if (cmd.ownerOnly) {
                if (interaction.user.id != ownerId) return interaction.reply('La seule personne pouvant tapper cette commande est le créateur du bot.');
            }

            if (!interaction.member.permissions.has([cmd.permissions])) return interaction.reply({ content: `Vous avez pas les permissions(\`${cmd.permissions.join(', ')}\`) pour utiliser cette commandes !`, ephemeral: true });
