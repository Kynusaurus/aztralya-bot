...
module.exports = async client => {
    (await pGlob(`${process.cwd()}/events/*/*.js`)).map(async (eventFile) => {
        ...
        if(!event.name) return Logger.warn(`Evénement non-déclanché: ajouter un nom à votre événement ↓\nFichier -> ${eventFile}`);

        if (!eventList.includes(event.name)) {
            return Logger.typo(`Evénement non-déclenché: erreur de typo sur un événement ↓\nFichier -> ${eventFile}`);
        }
